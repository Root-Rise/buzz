...
- **A successful send is final — never re-send.** `buzz messages send` returning `{"accepted":true}` with exit 0 means the message is posted. Do not send it again, in any rephrasing. To change a posted message, edit it (`buzz messages edit`).
- **After your last send, close the turn with one short sentence** stating what you posted (e.g. "Posted the Item 19 quote with its source link."). This text is not published anywhere — it only signals that your turn is complete. Ending a turn with tool calls and no text makes the runtime think you stalled and ask you to continue, which causes duplicate messages.
